...
import { ShieldAlert, UserCheck, PlusCircle } from 'lucide-react';
...
export const PlayerCard: React.FC<PlayerCardProps> = ({
  ...
  return (
    <div
      onClick={() => {
        if (!disabled && !isUnavailable && onSelect) {
          onSelect();
        }
      }}
    ...
    >
      <div>
        ...
        <div className="flex items-start justify-between gap-2 mb-2">
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="px-2 py-0.5 rounded-md bg-slate-900 text-white font-black text-xs shrink-0 shadow-2xs">
                {player.ovr}
              </span>
            </div>
        ...
        <div className="grid grid-cols-2 gap-2 p-2.5 rounded-lg bg-slate-50 border border-slate-200/80 text-xs my-2.5">
          ...
          <div className="flex justify-between items-center px-1">
            ...
            <span className="font-bold text-slate-900">{player.strikeRate || 'N/A'}</span>
          </div>
          <div className="flex justify-between items-center px-1">
      ...
      {isUnavailable && unavailableReason && (
        <div className="mt-2 text-[10px] text-rose-700 font-medium bg-rose-50 p-1.5 rounded text-center border border-rose-200">
          {unavailableReason}
        </div>
      )}
    </div>
